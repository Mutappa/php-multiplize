<div class="side_bar">
                <div class="dashboard_owner_container">
                    <img src="../../assets/images/logo-1.webp" alt="owner Name">
                </div>
                <ul class="sidebar_items">
                    <li class="tab1 a_tags showHideSubmenu">   
                        <a href="javascript:void(0);" class="dropDown showHideSubmenu">
                            <i class="fa-solid fa-list showHideSubmenu" aria-hidden="true"></i> 
                            Listings
                            <i class="fa fa-angle-left dropDownArrow showHideSubmenu"></i>
                        </a>
                        <ul class="subMenu">
                            <li>
                                <a class="subMenu_redirect" href="../tables/buyers.php"><i class="fa fa-circle-o"></i>Buyers</a>
                            </li>
                            <li>
                                <a href="../tables/renters.php" class="subMenu_redirect"><i class="fa fa-circle-o"></i>Renters</a>
                            </li>
                            <li>
                                <a class="subMenu_redirect" href="../tables/potential-buyers.php"><i class="fa fa-circle-o"></i>Potential Buyers</a>
                            </li>
                        </ul>
                    </li>  
                    <!-- <li class="tab2 a_tags">
                        <a href="../html/mainDashboard.php"><i class="fa-solid fa-database" aria-hidden="true"></i>
                        Dashboard
                        </a>
                    </li> -->
                </ul>
                <div class="logout_button">
                    <a href="../logout.php">
                        <i class="fa fa-power-off" aria-hidden="true"></i>
                    logout
                    </a>
                </div>
</div>